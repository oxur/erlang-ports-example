...
(defun handle_call
  ...
  ((msg from port)
   (let ((msg-bin (erlang:term_to_binary msg))
         (data (util:receive-line port (GO-TIMEOUT))))
...
(defun handle_info
  ...
  ((`#(EXIT ,from ,reason) port)
   (io:format "Go echo process ~p exited! (Reason: ~p)" `(,from ,reason))
   `#(noreply ,port))
  ((msg port)
   (logger:error "The Go echo server is handling info of type: ~p." `(,msg))
   `#(noreply ,port)))
...
(defun terminate
  ...
  ((shutdown _port)
   (logger:error "The supervisor is shutting down the Go echo server."))
  ((reason _port)
   (logger:error "The Go echo server is terminating for reason: ~p." `(,reason))))
