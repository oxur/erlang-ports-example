...
(defun handle_call
  ...
  ((msg from port)
   (let ((msg-bin (erlang:term_to_binary msg))
         (data (util:receive-line port (LISP-TIMEOUT))))
...
(defun handle_info
  ...
  ((`#(EXIT ,from ,reason) port)
   (io:format "Lisp echo process ~p exited! (Reason: ~p)" `(,from ,reason))
   `#(noreply ,port))
  ((msg port)
   (logger:error "The Lisp echo server is handling info of type: ~p." `(,msg))
   `#(noreply ,port)))
